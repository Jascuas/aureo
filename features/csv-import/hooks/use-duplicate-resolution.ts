...
type DuplicateResolutionState = {
  ...
  resolveAs: (csvIndex: number, action: 'skip' | 'import' | 'update') => void;
...
export const useDuplicateResolution = create<DuplicateResolutionState>((set, get) => ({
  isOpen: false,
  ...
  resolveAs: (csvIndex, action) => {
    set((state) => {
      const existingIndex = state.resolutions.findIndex(r => r.csvIndex === csvIndex);
  ...
  skipAllExact: (duplicates) => {
    ...
    set((state) => {
      ...
      exactDuplicates.forEach(dup => {
        const existingIndex = newResolutions.findIndex(r => r.csvIndex === dup.csvIndex);
        
        if (existingIndex >= 0) {
          newResolutions[existingIndex] = { csvIndex: dup.csvIndex, action: 'skip' };
        } else {
  ...
  getPendingCount: (duplicates) => {
    ...
    return duplicates.filter(dup => 
      !resolutions.find(r => r.csvIndex === dup.csvIndex)
    ).length;
  },
