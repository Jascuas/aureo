/**
 * Test script for OpenRouter AI Provider
 *
...
async function testOpenRouterProvider() {
  ...
  if (!apiKey) {
    ...
    console.error("Please set AI_API_KEY=your_openrouter_key in .env.local");
    process.exit(1);
  }
  ...
  try {
    ...
    columnResult.columns.forEach((col) => {
      console.log(
        `  - ${col.name} (index ${col.index}): ${col.detectedType} (confidence: ${(col.confidence * 100).toFixed(0)}%)`,
      );
    });
    ...
    categorizationResults.forEach((result) => {
      ...
      const categoryName =
        availableCategories.find(
          (c) => c.id === result.topSuggestion.categoryId,
        )?.name || "Unknown";
      console.log(`Transaction: ${tx.payee} (${tx.amount})`);
      console.log(
        `  → Top suggestion: ${categoryName} (${result.topSuggestion.categoryId}) (confidence: ${(result.topSuggestion.confidence * 100).toFixed(0)}%)`,
      );
    ...
    duplicateResults.forEach((result, idx) => {
      ...
      if (result.duplicates.length === 0) {
        ...
        result.duplicates.forEach((dup) => {
          console.log(
            `    • ${dup.matchType} match (similarity: ${(dup.similarity * 100).toFixed(0)}%)`,
          );
...
// Run tests
testOpenRouterProvider().catch((error) => {
