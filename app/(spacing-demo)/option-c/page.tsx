...
const NAV = [
  ...
  { href: "/option-b", label: "B · COMPACT", active: false },
  { href: "/option-c", label: "C · GHOST", active: true },
];
    ...
    <>
      ...
      <div className="border-border border-b px-4 py-2">
        <p className="text-success text-2xs tracking-widest uppercase">
          Option C — Ghost Cards
        </p>
        <p className="text-muted-foreground text-3xs mt-0.5">
          p-4 on wrapper · no card bg/border on mobile · sections separated by
          lines · 16px from edge
        </p>
      </div>

      {/* ─── CONTENT: normal outer padding, cards are transparent/section-style ─── */}
      <main className="p-4">
        {/* Stats — no card bg, just stacked with dividers */}
        <div className="border-border mb-4 border-y">
          {STATS.map((s, i) => (
            <div
              key={s.label}
              className={`py-4 ${i < STATS.length - 1 ? "border-border border-b" : ""}`}
            >
        ...
        {/* Transactions — section header + rows, no card container */}
        <div>
          <div className="border-border mb-0 border-b pb-2">
            <span className="text-muted-foreground text-2xs tracking-widest uppercase">
              Recent Transactions
            </span>
          </div>
          {TXS.map((tx, i) => (
            <div
              key={i}
              className={`flex items-center justify-between py-3 ${i < TXS.length - 1 ? "border-border border-b" : ""}`}
            >
              <div>
