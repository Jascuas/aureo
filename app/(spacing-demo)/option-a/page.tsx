...
const NAV = [
  { href: "/option-a", label: "A · EDGE BLEED", active: true },
    ...
    <>
      ...
      <div className="border-border border-b px-4 py-2">
        ...
        <p className="text-muted-foreground text-3xs mt-0.5">
          px-0 on wrapper · cards full-width · p-4 internal · 16px from edge
        </p>
      </div>

      {/* ─── CONTENT: no horizontal padding on wrapper ─── */}
      <main className="px-0 py-4">
        {/* Stats grid */}
        <div className="mb-4 grid grid-cols-1 gap-px">
          {STATS.map((s) => (
            <div
              key={s.label}
              className="bg-card border-border border-x-0 border-y px-4 py-4"
            >
        ...
        {/* Transactions card */}
        <div className="bg-card border-border border-x-0 border-y">
          ...
          {TXS.map((tx, i) => (
            <div
              key={i}
              className={`flex items-center justify-between px-4 py-3 ${i < TXS.length - 1 ? "border-border border-b" : ""}`}
            >
              <div>
